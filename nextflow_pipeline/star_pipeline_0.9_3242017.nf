...
{       println("index does not exits")
		...
		cache 'deep'
                ...
                output:
                file 'genome_index.*.bt2' into bowtieindex
	

                """
		mkdir ${params.refgenome}/bowtie	
                bowtie2-build -c fasta genome_index
...
{               process Kallisto_index {
		...
		cache 'deep'
                ...
                output:
                file "transcriptome.index" into tindex
...
if(params.aligner == 'star')
{
	process star {
		...
		cache 'deep'
    		...
    		output:
    		set sampleid, file('*Log.final.out'), file ('*.bam') into bam_files, bam_count, bam_reseqc, bam_rseqc_bamstats, bam_rseqc_stats, bam_rseqc_genecoverage, bam_stringtieFPKM
...
process rseqc {
  ...
  clusterOptions = "-P ${params.project} -l h_rt=96:00:00 -l mem_total=5G -pe omp 12"
	...
	set sampleid, bamfiles1 from bam_rseqc_genecoverage
	file gtf from bed
	...
	geneBody_coverage.py -r  gtf -i bamfiles1 -o gene_coverage
